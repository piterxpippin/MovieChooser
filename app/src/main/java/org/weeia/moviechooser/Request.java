...
public class Request extends AppCompatActivity {
    ...
    ElasticSearchMovieSource elasticSearchMovieSource;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //elasticSearchMovieSource = new ElasticSearchMovieSource("localhost",9300);
            ...
            public void onClick(View view) {
                if(title.getText().toString().contentEquals("")){
                    if(exactRate.isChecked()){
                        //movieList = elasticSearchMovieSource.getMoviesWithGivenGenreAndAverageRatingStratingFrom(movieType, ratingbar.getRating(),0,50,20);
                        Snackbar.make(view, "search by exaxt", Snackbar.LENGTH_LONG)
                                .setAction("Action", null).show();
                    }else {
                        //movieList = elasticSearchMovieSource.getMoviesWithGivenGenreAndMinimumNumberOfRatings(movieType, Math.round(ratingbar.getRating()), 0, 50);
                        Snackbar.make(view, "search my min", Snackbar.LENGTH_LONG)
                                .setAction("Action", null).show();
                    }
                }else{
                    //movieList.add(elasticSearchMovieSource.getMovieByTitle(title.getText().toString()));
                    Snackbar.make(view, "search by title", Snackbar.LENGTH_LONG)
                            .setAction("Action", null).show();
                }
